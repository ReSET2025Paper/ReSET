"""
This script preprocesses robot play data and human teleoperation data, 
aligning them using flow tracking to construct pairs for correction policy training.

Data Structure Formats (outputs generated by preprocess/preprocess_track.py):

1. Human Data (*_tracking.pkl)
   - A pickled dictionary containing:
     - 'observations': List[np.ndarray], where each element is a video sequence of shape (T, H, W, 3)
     - 'tracking': List[np.ndarray], where each element is an optical flow trajectory of shape (T, N, 2)
     - 'text_cond' (optional): np.ndarray representing the encoded text conditioning vector

2. Robot Play Data (*_tracking.pkl)
   - A pickled dictionary containing:
     - 'type': str, interaction category (["pull", "pick", "rotate"])
     - 'img': np.ndarray, video sequence of shape (T, H, W, 3)
     - 'gripper_img' (optional): np.ndarray, gripper video sequence of shape (T, H, W, 3)
     - 'joint_vel' (optional): np.ndarray of joint velocities of shape (T, 7)
     - 'joint_state': np.ndarray of robot joint positions of shape (T, 7)
     - 'tracking': np.ndarray of optical flow trajectory of shape (T, N, 2)
"""
import os
import cv2
import time
...
def get_closest_action_flow_pair(flow: np.array,  
                                 ...
                                 play_data: np.array, 
                                 horizon: int = 18) -> tuple[int, np.array, np.array]:
...
def get_interaction_specs_for_obj(robot_demo: dict, robot = None) -> dict:
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.task != 'all': # preprocess a single task
        data = {"img": [],
                "correction_specs": []}
    ...
    else: # combine all tasks since the model will be task agnostic
        if cfg.playdata_type == 'preset':
            tasks = ["drop_bowl", "pick_block", "sort_screw", "multi_task"]
            save_path = os.path.dirname(cfg.save_path)
            play_data = {"img": [],
                        "correction_specs": []}
            for task in tasks:
